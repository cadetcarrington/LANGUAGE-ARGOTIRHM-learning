...
void addMatrixOpenMp(double* A, const double* B, int M) {
#if defined(_OPENMP)
...
bool parseSize(int argc, const char* argv[], int& M, int& N) {
    ...
    if (argc == 1 || std::strcmp(argv[1], "1024") == 0) {
        std::cout << "RUNNING OPENMP DGEMM: M = 1024" << std::endl;
        return true;
    }
    if (argc == 2 && std::strcmp(argv[1], "4096") == 0) {
        M = 4096;
        N = 4096;
        std::cout << "RUNNING OPENMP DGEMM: M = 4096" << std::endl;
        return true;
    }
...
int main(int argc, const char* argv[]) {
    ...
    dgemmOpenMpBlocked(A.data(), B.data(), res.data(), M, N);
    addMatrixOpenMp(res.data(), C.data(), M);
